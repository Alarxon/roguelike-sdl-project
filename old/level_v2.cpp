...
vector<vector<char>> gen_level(int w, int h, int max_r, int min_r_xy, int max_r_xy, bool r_overlap, int random_con, int random_spur){
    ...
    /*
    //sort
    sort(room_list.begin(), room_list.end(), [](const vector<int> &a, const vector<int> &b) {
    ...
    */
    ...
    for (int i = 0; i < (int)corridor_list.size(); i++){
        ...
        for (int j = 0; j < width_range; j++){
            ...
            for (int k = 0; k < height_range; k++){
                if(level[min(corridor_1.second, corridor_2.second) + k]
                    [min(corridor_1.first, corridor_2.first) + j] != 'D'){
                        level[min(corridor_1.second, corridor_2.second) + k]
                            [min(corridor_1.first, corridor_2.first) + j] =  FLOOR;
                }else{
                    level[min(corridor_1.second, corridor_2.second) + k]
                            [min(corridor_1.first, corridor_2.first) + j] =  'E';
                }
            }
        }
        ...
        if (corridor_list[i].size() == 3){
            ...
            for (int j = 0; j < width_range; j++){
                ...
                for (int k = 0; k < height_range; k++){
                    if(level[min(corridor_2.second, corridor_3.second) + k]
                         [min(corridor_2.first, corridor_3.first) + j] != 'D'){
                            level[min(corridor_2.second, corridor_3.second) + k]
                                [min(corridor_2.first, corridor_3.first) + j] = FLOOR;
                    }else{
                        level[min(corridor_2.second, corridor_3.second) + k]
                            [min(corridor_2.first, corridor_3.first) + j] = 'E';
                    }
    ...
    for (int i = 1; i < (height - 1); i++){
        for (int j = 1; j < (width - 1); j++){
            if (level[i][j] == FLOOR){
                if (level[i - 1][j - 1] == STONE || level[i - 1][j - 1] == DOOR)
                    level[i - 1][j - 1] = WALL;
                if (level[i - 1][j] == STONE || level[i - 1][j] == DOOR)
                    level[i - 1][j] = WALL;
                if (level[i - 1][j + 1] == STONE || level[i - 1][j + 1] == DOOR)
                    level[i - 1][j + 1] = WALL;
                if (level[i][j - 1] == STONE || level[i][j - 1] == DOOR)
                    level[i][j - 1] = WALL;
                if (level[i][j + 1] == STONE || level[i][j + 1] == DOOR)
                    level[i][j + 1] = WALL;
                if (level[i + 1][j - 1] == STONE || level[i + 1][j - 1] == DOOR)
    ...
    //paint doors
    for(int i = 0; i < (int)room_list.size(); i++){
        int top_start_x = room_list[i][0] - 1;
        int top_start_y = room_list[i][1] - 1;
        for(int j = top_start_x; j < top_start_x + room_list[i][2]; j++){
            if(level[top_start_y][top_start_x] == FLOOR){
                level[top_start_y][top_start_x] = DOOR;
            }
            top_start_y++;
        }
    }
